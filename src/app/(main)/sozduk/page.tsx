...
import { BookOpen, Filter, ArrowUpDown, ChevronLeft, ChevronRight } from 'lucide-react'
...
type Filters = { query: string; activeCategory: string; sortOrder: SortOrder; page: number }
type FilterAction =
...
function filtersReducer(state: Filters, action: FilterAction): Filters {
  switch (action.type) {
    ...
    case 'SET_CATEGORY': return { ...state, activeCategory: action.category, page: 0 }
    case 'SET_SORT':     return { ...state, sortOrder: action.sortOrder, page: 0 }
...
function SozdukContent() {
  ...
  const [filters, dispatch] = useReducer(filtersReducer, {
    ...
    activeCategory: 'Баары',
    sortOrder: 'az',
    page: 0,
  })
  const { query, activeCategory, sortOrder, page } = filters

  const [entries, setEntries] = useState<SozdukEntry[]>([])
  const [totalCount, setTotalCount] = useState(0)
  const [loading, setLoading] = useState(true)

  // Reset to page 0 when lang changes
  const prevLang = useRef(langFilter)
  useEffect(() => {
    if (prevLang.current !== langFilter) {
      prevLang.current = langFilter
  ...
  useEffect(() => {
    ...
    const translationCol = langFilter === 'kg-ru' ? 'word_ru' : 'word_en'
    ...
    async function fetchPage() {
      ...
      if (activeCategory !== 'Баары') q = q.eq('category', activeCategory)

      if (query) {
        // Fetch up to 1000 matches then rank by relevance client-side so exact /
        // starts-with results always surface before arbitrary "contains" hits.
        q = q
          .or(`word_kg.ilike.%${query}%,${translationCol}.ilike.%${query}%,example_kg.ilike.%${query}%`)
        ...
        const rank = (e: SozdukEntry) => {
          ...
          if (kg.includes(ql))     return 3
          return 4
        }
        const ranked = [...(data as SozdukEntry[] || [])].sort((a, b) => rank(a) - rank(b))
        setEntries(ranked.slice(page * PAGE_SIZE, (page + 1) * PAGE_SIZE))
        setTotalCount(Math.min(count ?? 0, ranked.length))
        setLoading(false)
        return
      }

      // No search — pure server-side pagination with selected sort
      switch (sortOrder) {
        case 'az':     q = q.order('word_kg',    { ascending: true });  break
        case 'za':     q = q.order('word_kg',    { ascending: false }); break
      ...
      if (!controller.signal.aborted) {
        setEntries((data || []) as SozdukEntry[])
  ...
  }, [page, query, activeCategory, sortOrder, langFilter])
        ...
        <div className="flex gap-2">
          <div className="flex-1">
            <SearchBar
              ...
              onSearch={(q) => dispatch({ type: 'SET_QUERY', query: q })}
            />
          </div>
          <div className="flex items-center gap-1.5 shrink-0 px-3 rounded-xl border border-border bg-background text-sm text-muted-foreground cursor-pointer">
            ...
            <select
              value={sortOrder}
              onChange={e => dispatch({ type: 'SET_SORT', sortOrder: e.target.value as SortOrder })}
        ...
        <>
          ...
          <div className="flex items-center justify-between mt-8" style={{ fontFamily: 'var(--font-nunito)' }}>
            <p className="text-muted-foreground/50 text-sm">
              {totalCount} сөз · {page + 1} / {totalPages}
            </p>
              ...
              {Array.from({ length: totalPages }, (_, i) => i)
                ...
                .map((item, idx) =>
                  item === '…' ? (
                    ...
                    <button
                      key={item}
                      onClick={() => dispatch({ type: 'SET_PAGE', page: item as number })}
